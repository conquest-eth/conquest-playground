...
function useDotEnvFromNetwork(network) {
  let envs = ['.env', 'contracts/.env', 'subgraph/.env'];
  if (network && network !== 'localhost') {
    if (fs.existsSync(`.env.web.${network}`) || fs.existsSync(`.env.${network}`)) {
      envs = [];
      if (fs.existsSync(`.env.${network}`)) {
        envs.push(`.env.${network}`);
      }

      if (fs.existsSync(`.env.web.${network}`)) {
        envs.push(`.env.web.${network}`);
      }

      envs.push(`.env`);
      envs.push(`contracts/.env`);
      envs.push(`subgraph/.env`);
      envs.push(`web/.env`);
    }
  }
  console.log({envs});
